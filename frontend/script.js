const API_BASE_URL = '';
const CLIENT_ID_KEY = 'urlShortenerClientId';
...
async function shortenUrl(mode) {
  ...
  try {
    const response = await clientFetch('/api/shorten', {
      method: 'POST',
      headers: {
...
async function loadUrlList() {
  try {
    const response = await clientFetch('/api/urls');
...
async function deleteUrl(id) {
  ...
  try {
    const response = await clientFetch(`/api/urls/${encodeURIComponent(id)}`, {
      method: 'DELETE'
    });
