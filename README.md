...
Spring Boot와 MySQL로 URL 매핑 및 클릭 통계를 관리하며, 브라우저에 저장한 익명 클라이언트 ID로 컴퓨터·브라우저별 URL 목록을 분리합니다. 로컬 환경은 Docker Compose로 실행하고, AWS 환경은 Terraform으로 구성하며 CloudFront, S3, ALB, EC2, RDS를 이용한 3-tier 아키텍처를 사용합니다.
...
사용자가 `www.url-shortener.p-e.kr`로 접속하면 S3에서 제공하는 프론트엔드가 브라우저의 `localStorage`에 익명 UUID를 생성합니다. CloudFront는 요청 경로에 따라 트래픽을 구분하며 HTML, CSS, JavaScript는 S3에서 제공합니다. `/api/*` 요청에는 UUID를 `X-Client-ID` 헤더로 포함하고, 공개 리다이렉트 경로인 `/s/*` 요청과 함께 ALB로 전달합니다.

ALB는 `/health` 상태 검사를 통과한 EC2 인스턴스에 요청을 분산하며, 각 EC2는 시작될 때 ECR에서 최신 Docker 이미지를 가져와 Spring Boot 애플리케이션을 컨테이너로 실행합니다. 백엔드는 `X-Client-ID`가 올바른 UUID 형식인지 검사하고 이를 소유자 ID로 사용합니다. 생성된 단축 URL과 원본 URL 매핑 정보, 클릭 횟수 및 소유자 ID는 RDS MySQL에 저장합니다.
...
CloudFront는 요청 경로에 따라 Origin을 구분합니다. 기본 경로의 `index.html`, `style.css`, `script.js`는 S3에서 제공하고, `/api/*`와 `/s/*` 요청은 ALB로 전달합니다. `/api/*` 요청에는 브라우저 UUID가 `X-Client-ID` 헤더로 포함되며 CloudFront가 이 헤더를 ALB와 백엔드까지 전달합니다. API 응답과 리다이렉트 결과가 캐시에 남지 않도록 두 경로의 TTL을 `0`으로 설정했습니다.

ALB는 요청을 Target Group에 등록된 EC2 인스턴스의 `5000` 포트로 전달합니다. Target Group은 `/health` 경로를 통해 각 인스턴스의 상태를 확인하며, 상태가 `healthy`인 인스턴스에만 요청을 분산합니다. 백엔드는 `X-Client-ID`와 DB의 `owner_id`가 일치하는 URL만 조회하거나 변경합니다. `/health`와 `/s/*`에는 클라이언트 ID가 필요하지 않습니다.
...
브라우저 UUID는 로그인 기능이 아닌 목록 분리용 식별자입니다. `localStorage`를 삭제하거나 시크릿 모드 또는 다른 브라우저를 사용하면 새로운 목록이 생성되며, 여러 컴퓨터 사이에서 목록이 동기화되지 않습니다. `X-Client-ID`는 인증 자격 증명이 아니므로 높은 수준의 접근 제어가 필요한 서비스에서는 별도의 사용자 인증을 추가해야 합니다.
...
| Method | Endpoint | `X-Client-ID` | 설명 |
|---|---|---|---|
| `POST` | `/api/shorten` | 필요 | 현재 브라우저의 단축 URL 생성 |
| `GET` | `/s/{shortCode}` | 불필요 | 원본 URL로 리다이렉트하고 클릭 수 증가 |
| `GET` | `/api/urls` | 필요 | 현재 브라우저가 생성한 URL 목록 조회 |
| `GET` | `/api/stats/{shortCode}` | 필요 | 현재 브라우저 URL의 생성 시각과 클릭 수 조회 |
| `DELETE` | `/api/urls/{id}` | 필요 | 현재 브라우저가 저장한 URL 삭제 |
