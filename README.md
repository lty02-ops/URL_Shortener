...
사용자가 `www.url-shortener.p-e.kr`로 접속하면 CloudFront가 모든 요청을 먼저 받습니다. CloudFront는 요청 경로에 따라 트래픽을 구분하며, HTML, CSS, JavaScript 같은 정적 파일은 S3에서 제공합니다. 단축 URL 생성·조회·삭제에 사용하는 `/api/*` 요청과 원본 URL로 리다이렉트하는 `/s/*` 요청은 ALB로 전달합니다.

ALB는 `/health` 상태 검사를 통과한 EC2 인스턴스에 요청을 분산하며, 각 EC2는 시작될 때 ECR에서 최신 Docker 이미지를 가져와 Spring Boot 애플리케이션을 컨테이너로 실행합니다. 생성된 단축 URL과 원본 URL 매핑 정보, 클릭 횟수는 RDS MySQL에 저장합니다.
...
CloudFront는 요청 경로에 따라 Origin을 구분합니다. 기본 경로의 HTML, CSS, JavaScript 파일은 S3에서 제공하고, `/api/*`와 `/s/*` 요청은 ALB로 전달합니다. API 응답과 리다이렉트 결과가 캐시에 남지 않도록 두 경로의 TTL을 `0`으로 설정했습니다.

ALB는 요청을 Target Group에 등록된 EC2 인스턴스의 `5000` 포트로 전달합니다. Target Group은 `/health` 경로를 통해 각 인스턴스의 상태를 확인하며, 상태가 `healthy`인 인스턴스에만 요청을 분산합니다.
